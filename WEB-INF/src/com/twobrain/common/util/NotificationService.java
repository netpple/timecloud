...
public class NotificationService {
    ...
    public boolean sendEmailFindNotification(final String email, final String password){
        return Notification.sendEmailFindPassword(email,password);
    }
...
class Notification {
    ...
    @Setter
    @Getter
    public ArrayList<String> ntfcTargetUser;
    ...
    @Setter
    @Getter
    public int taskOwnerIdx;

    // 노티피케이션 전송 사용자 IDX
    @Setter
    @Getter
    public int ntfcSenderIdx;

    // 노티피케이션이 발생된 테이블(도구) IDX
    @Setter
    @Getter
    public int ntfcTableIdx;

    // 노티피케이션 전송 사용자 명
    @Setter
    @Getter
    public String ntfcSenderName;

    // 노티피케이션이 발생된 테이블(도구) 명
    @Setter
    @Getter
    public String ntfcTableName;
    ...
    @Setter
    @Getter
    public Map<String, Object> ntfcMetaInfo;

    // 노티피케이션 타입
    @Setter
    @Getter
    public NotificationType ntfcType;

    // 사용자 세션 정보
    @Setter
    @Getter
    public transient UserSession userSession;
    ...
    /**
     * @param email    - 임시 비밀번호를 전송할 사용자 이메일
     * @param password - 임시 비밀번호
     */
    public static boolean sendEmailFindPassword(final String email, final String password) {

        if (StringUtils.isEmpty(email) || StringUtils.isEmpty(password))
            return false;

        boolean isSend = false;
        String title = "태스크투게더 비밀번호 안내입니다.";
        String body = String.format("현재 발급된 임시 비밀번호는 %s 입니다. 로그인 후 비밀번호를 반드시 변경해 주세요.",password);
