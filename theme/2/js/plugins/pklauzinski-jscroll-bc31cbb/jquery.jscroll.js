...
(function ($) {
    ...
    $.jscroll = {
        defaults: {
            ...
            pagingSelector: '',
            callback: false,
            jsonParser : false // added by Sam Kim for Json Parsing
        }
    };
...
//                }
        ...
        function _load() {
            ...
            return $e.animate({scrollTop: $inner.outerHeight()}, 0, function () {
                // added by Sam Kim for JSon Parsing
                if(_options.jsonParser){
                    $.getJSON(data.nextHref, function (result) {
                        _options.jsonParser.call(this,result);
                    }).always(function(result){
                        var lastEl = $inner.find('div.jscroll-added').last();
                        lastEl.html(""); // erase loading ..
                        lastEl.append($("<a></a>",{'href':result.nexturl,'class':'jscroll-next'}));
                        var $next = lastEl.find(_options.nextSelector).first();
                        data.waiting = false;
                ...
                $inner.find('div.jscroll-added').last().load(data.nextHref, function(r, status, xhr) { // data loaded from here
                    if (status === 'error') {
                        return _destroy();
                    }
