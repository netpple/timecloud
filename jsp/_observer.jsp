<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%><%@ page import="java.util.*,java.io.*" %><%@ include file="./common/include/incInit.jspf" %><%@ include file="./common/include/incSession.jspf" %><%@ include file="./common/include/incTaskView.jspf" %><%
	...
	if(!isAll && req.getIntParam("owner_idx") == oUserSession.getUserIdx() ){
		 
		String userCombo = getUserCheckBox(oUserSession,TASK_IDX) ;
		StringBuffer sbForm = new StringBuffer() ;
			sbForm.append("<div class='form-inline'>")
			.append(userCombo)
			.append("<div class='row-fluid show-grid'>")
				.append("<textarea id='pComment' placeholder='참조자에게 남기실 한마디 ..' rows=2 class='span5'></textarea>")
				.append(" <button onclick='javascript:onSubmitObserver();' class='btn btn-primary'>등록</button>")
			.append("</div>")
			.append("</div>");
		sb.append(sbForm) ;
	}
...
<script type="text/javascript">
	function observer_delete(n_task_idx, n_idx) {
		if( confirm("삭제하시겠습니까?") ){
			...
			$.ajax({	
				type:"POST",
				url:"observerDeleteAction.jsp?tsk_idx="+n_task_idx+"&idx="+n_idx,
				success:function(data){	
					if(data >0){
						$.ajax({
							type : 'post',
							async : true,
							url : "observer.jsp?tsk_idx="+<%=TASK_IDX%>+"&owner_idx="+<%=req.getParam("owner_idx")%>,
							beforeSend : function() {
							},
							success : function(data) {
								$('#taskContent *').remove();
	...
	function onSubmitObserver() {
		if(lock())return false ;
		
		var observer = $.param($("#observerCheckList").find("input[name=pObserver]:checked"));
		...
		$.ajax({	
			type:"POST",
			url:'observerAction.jsp?tsk_idx='+<%=TASK_IDX%>+'&'+observer+'&pComment='+comment.val(),
			success:function(data){	
				if(data >0){
					$.ajax({
						type : 'post',
