...
$(document).ready(function(){ 
    ...
    $(".nav-tabs > li" ).click(function() {
    	if($(this).find("a").attr("href") == '#activityTab') {
    		...
    		$("#taskMenu").find("li").removeClass("active");
    		$("#menu_calendar").addClass("active");
    		$.ajax({
    			type : 'post',
    			async : true,
    			url : "calendar.jsp?tsk_idx="+<%=TASK_IDX%>+"&owner_idx="+<%=req.getParam("owner_idx")%>+"&modal=Y",
    			beforeSend : function() {
    			},
...
function onFeedbackAdd() {
	if(lock())return false ;
	var pFeedback = $( "#pFeedback");
	if(pFeedback.val()=="") {
		alert('내용을 입력해 주세요..') ; 
		pFeedback.focus() ;
		unlock() ;
...
function feedbackAdd() {
	$.ajax({
		type : 'post',
		async : true,
		url : "feedbackAction.jsp?tsk_idx="+<%=TASK_IDX%>+"&pFeedback="+ $( "#pFeedback").val()+"&feedType=GAP",
		beforeSend : function() {
		},
		success : function(data) {
			$("#toolContent").prepend(data);	
		},
		complete : function() {
		},
		error : function(e){
...
function tool_delete(taskIdx, toolIdx, toolType) {
	if(confirm("삭제하시겠습니까?")) {
		$.ajax({
			type : 'post',
			async : true,
			url : "taskHomeAction.jsp?task_idx="+taskIdx+"&tool_idx="+toolIdx+"&tool_type="+toolType,
			beforeSend : function(){
			},
			success : function(data){
				$("#"+toolType+"_"+toolIdx).remove();
			},
			complete : function(){
			}
		});
	...
	<div class="tabbable"> <!-- Only required for left/right tabs -->
		...
		<div class="tab-content">
			<div class="tab-pane active inputContainer" id="tab1">
				<div class='form-inline row-fluid show-grid'>
					<textarea class='textInput animated' name='pFeedback' id="pFeedback" placeholder='피드백을 남겨주세요 ..' rows=2></textarea>
				</div>
				<div align=right style='background:#f3f3f3'><button onclick="javascript:onFeedbackAdd();" style="align:right"  class='btn btn-primary'>등록</button></div>
			</div>
	...
	public String get() {
		...
		try {
			...
			if("FILE".equals(v_type)) {
				sbOut.append( 
					Html.a(
						Html.img_(String.format( "onerror=\"this.src='/repos/2brain.com/thumbnail/file_icon.png'\" src='/jsp/common/fileupload/fileAction.jsp?pAction=DownloadFile&pFileIdx=%d&pThumbnail=true'",n_idx ) )
						+ Html.br_("") + v_desc
