...
var myTree = new AXTree();
				...
				menu:[
				    {isOpen:true, label:"액티비티/할당", className:"plus", onclick:function(){
				    	...
				    	$.ajax({
				    		type : 'post',
				    		async : true,
				    		url : "taskMenu.jsp?tsk_idx="+this.sendObj.no+"&owner_idx="+<%=oTaskHierarchy.getCurrentTask().getOwnerIdx()%>+"&view=calendar&modal=Y",
				    		beforeSend : function() {
				    		},
				    		success : function(data) {
				    			$('#taskMenu *').remove();
		...
		// -- console.debug(treeJson) ;
 		...
 		function onTaskAllAction(){
 			...
 			$.ajax({	
 				type:"POST",
 				url:'taskMenu.jsp?tsk_idx='+<%=Html.trueString(taskList>0,taskList+"+'&tsk_list="+taskList+"'")%> + "&owner_idx="+<%=oTaskHierarchy.getCurrentTask().getOwnerIdx()%> ,
 				beforeSend: function() {  
 					$("#taskMenu *").remove();
		        },  
				success:function(data){	
